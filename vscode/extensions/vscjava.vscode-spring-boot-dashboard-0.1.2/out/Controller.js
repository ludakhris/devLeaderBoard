...
class Controller {
    ...
    startBootApp(app, debug) {
        return __awaiter(this, void 0, void 0, function* () {
            const mainClasData = yield vscode.window.withProgress({ location: vscode.ProgressLocation.Window, title: `Resolving main classes for ${app.name}...` }, () => { return this._getMainClass(app.path); });
            if (mainClasData) {
                ...
                const ok = yield vscode.debug.startDebugging(vscode.workspace.getWorkspaceFolder(vscode.Uri.parse(app.path)), Object.assign({}, targetConfig, {
                    noDebug: !debug,
                    vmArgs
                }));
                if (ok) {
    ...
    _getMainClass(folder) {
        return __awaiter(this, void 0, void 0, function* () {
            // Note: Command `vscode.java.resolveMainClass` is implemented in extension `vscode.java.resolveMainClass`
            const mainClassList = yield vscode.commands.executeCommand('java.execute.workspaceCommand', 'vscode.java.resolveMainClass', folder);
            if (mainClassList && mainClassList instanceof Array && mainClassList.length > 0) {
                return mainClassList.length === 1 ? mainClassList[0] :
                    yield vscode.window.showQuickPick(mainClassList.map(x => Object.assign({ label: x.mainClass }, x)));
            }
